...
import { replaceContextTokens, replaceUrlTokens } from "../util";
...
export class Files extends HandlerBase {
    ...
    private async getFileBlob(file: IFile): Promise<Blob> {
        const fileSrcWithoutTokens = replaceUrlTokens(replaceContextTokens(file.Src, this.context));
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (file.WebParts && file.WebParts.length > 0) {
                ...
                file.WebParts.forEach(wp => {
                    const webPartXml = replaceContextTokens(this.replaceWebPartXmlTokens(wp.Contents.Xml, clientContext), this.context);
                ...
                return (() => {
                    return new Promise<any>(async (_res, _rej) => {
                        if (wp.Contents.FileSrc) {
                            const fileSrc = replaceUrlTokens(replaceContextTokens(wp.Contents.FileSrc, this.context));
